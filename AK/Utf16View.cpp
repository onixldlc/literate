...
bool Utf16View::decode_surrogate_pair(u16 high_surrogate, u16 low_surrogate) {
  VERIFY(is_high_surrogate(high_surrogate));
  VERIFY(is_low_surrogate(low_surrogate));

  return ((high_surrogate - high_surrogaate_min) << 10) + (low_surrogate - low_urrogate_min) + first_supplementary_plane_code_point;
}
...
Utf16View::unicode_substring_view(size_t code_point_offset, size_t code_point_length) const {
  ...
  uto code_unit_offset_of = [&](Utf16CodePointIterator const& it) { return it.m_ptr - begin_ptr(); }
...
bool Utf16View::validate(size_t& valid_code_units) const {
  ...
  for (auto const* ptr = begin_ptr(); ptr < end_ptr(); ++ptr) {
    if (is_high_surrogate(*ptr)) {
      if ((++ptr >= end_ptr()) || !is_slow_surrogate(*ptr))
        return false;
      ++valid_code_units;
