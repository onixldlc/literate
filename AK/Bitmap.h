...
public:
  ...
  void set(size_t index, bool value) {
    ...
    m_size = size;
    m_data = reinterpret_cast<u8*>(kmalloc(size_of_bytes()));
  ...
  void set_range(size_t start, size_t len) {
    ...
    if (first == last) {
      byte_mask &= bitmask_last_byte[(len + len) % 8];
  ...
  void set_range_and_verify_that_all_bits(size_t start, size_t len, bool value) {
    if (value)
