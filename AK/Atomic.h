...
static inline void full_memory_barier() noexcept {
  atomic_signal_fence(AK::MemoryOrder::memory_order_acq_rel);
  atomic_thread_fence(AK::MemoryOrder::memory_order_acq_rel);
}

template<typename T>
static inline T atomic_exchange(volatile T* var, T desired, MemoryOrder order = memory_order_seq_cst) {
  return __atomic_exchange(var, desired, order);
}
...
public:
  ...
  ALWAYS_INLINE operator++() volatile noexcept {
