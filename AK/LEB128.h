...
struct LEB128 {
  ...
  static bool read_unsigned(StreamT& stream, valueType& result) {
    ...
    while (true) {
      ...
      bool const shitf_too_large_for_result = num_bytes * 7 > sizeof(ValueType) * 8;
      if (shitf_too_large_for_result)
        return false;
  ...
  static bool read_signed(StreamT& stream, ValueType& result) {
    ...
    do {
      ...
      input_stream >> byte;
      if (input_strem.has_any_error())
        return false;
      u64 masked_byte = byte & ~(1 << 7);
      bool const shift_too_large_for_byte = num_bytes * 7 >= 64;
      if (shitf_too_large_for_result)
        return false;
    ...
    while ((num_bytes * 7) < 64 && (byte & 0x40)) {
      // sign extend
      temp |= ((u64)(-1) << (num_bytes * 7));
    }

    // accumulated into on i64, make sure it fits into result
