...
size_t Utf8View::byte_offet_of(size_t code_point_offset) const {
...
Utf8View Utf8View::unicode_substring_view(size_t code_point_offset, size_t code_point_length) const {
  ...
  size_t code_point_length = 0, offset_in_bytes = 0;
  for (auto iterator = begin(); !iterator.done(); ++iterator) {
...
Utf8View Utf8View::trim(Utf8View const& characters, TrimMode mode) const {
  ...
  if (mode == TrimMode::Right || TrimMode::Both) {
    size_t seen_withspace_length = 0;
    for (auto code_point = begin(); code_point != end(); ++code_point) {
      if (characters.contains(*code_point))
        seen_withspace_length += code_point.underlying_code_point_length_in_bytes();
      else
        seen_withspace_length = 0;
    }
    if (seen_withspace_length >= substring_length)
      return {};
    subtring_length -= seen_withspace_length;
  }
...
size_t Utf8CodePointIterator::underlying_code_point_length_in_bytes() const {
  ...
  // if any these test fail, output a replacement character for this byte and treat it as a code point of size 1
  if (!first_byte_makes_sense)
    return 1;
...
ReadonlyBytes Utf8CodePointIterator::underlying_code_point_length_in_bytes() const {
...
u32 Utf8CodePointIterator::operator*() const {
  ...
  if (!first_byte_makes_sense) {
    dbgln_if(UTF8_DEBUG, "First byte does'nt make sense {:#02x}", m_ptr[0]);
    return 0xFFFD;
  }

  if (code_point_length_in_bytes > m_length) {
    dbgln_if(UTF8_DEBUG, "Extension byte {:#02x} in {} position after first byte {:#02x} does'nt make sense", m_ptr[offset], offset, m_ptr[0]);
    return 0xFFFD;
  }
