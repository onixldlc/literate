...
public:
  ...
  ALWAYS_INLINE NonnullRefPtr(T const& object) : m_ptr(const_cast<T*>(&object)) {
    m_ptr -> ref();
  }
  ...
  ALWAYS_INLINE NonnullRefPtr(AdoptTag, T& object) : m_ptr(&object) {
    // TODO: implemented
  }

  ALWAYS_INLINE NonnullRefPtr(NonnullRefPtr&& other) : m_ptr(&other.leak_ref()) {
    // TODO: implemented
  }
  ...
  ALWAYS_INLINE NonnullRefPtr(NonnullRefPtr const& other) : m_ptr(const_cast<T*>(other.ptr())) {
    m_ptr -> ref();
  }
...
requires(IsConvertible<T, Args...>) inline NonnullRefPtr<T> make_ref_counted(Args&&... args) {
