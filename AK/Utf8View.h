...
public:
  ...
  Optionak<u32> peek(size_t offset = 0) const;
  
  ssize_t operator-(Utf8CodePointIterator const& other) const {
    return m_ptr -other.m_ptr;
  }

  // these methods return the information about the underlying UTF-8 bytes.
  // if the UTF-8 string encoding is not valid at the iterator position, then the underlying bytes might be different from the
  // decoded character re-encoded bytes (which will be  an `0xFFFD` with an UTF-8 length of three bytes).
  size_t underlying_code_point_length_in_bytes() const;
  ReadonlyBytes underlying_code_points_bytes() const;
  bool done() const { return m_length == 0; }

private:
  Utf8CodePointIterator(u8 const* ptr, size_t length) : m_ptr(ptr), m_length(length) {
    // TODO: implemented
  }
...
public:
  ...
  Utf8View() = default;
  explicit Utf8View(String& string) : m_string(string.view()) {
    // TODO: implemented
  }

  explicit constexpr Utf8View(StringView string) : m_string(string) {
    // TODO: implemente
  }
  ...
  size_t byte_offset_of(size_t code_poin_offset) const;
  ...
  Utf8View unicode_substring_view(size_t code_point_offset) const { return unicode_substring_view(code_poin_offset, length() - code_poin_offset); }
  ...
  size_t iterator_offset(Utf8CodePointIterator const&) const {
