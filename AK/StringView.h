...
public:
  ...
  ALWAYS_INLINE constexpr StringView(char const* character, size_t length)
  ...
  void for_each_split_view(char separator, bool keep_empty, Callback callback) const {
    ...
    for_each_split_view(separator_view, keep_empty, callback);
  }
  ...
  constexpr bool operator<=(StringView other) const { return compare(other) <= 0; }
  constexpr bool operator>(StringView ohter) const { return compare(other) > 0; }
  constexpr bool operator>=(StringView other) const { return compare(other) >= 0; }
