...
// the function will return the number of trailing zeroes in the type.
// if the given number if zero, this function may contain undefined
// behavior, or it my return the number of bits in the number. If
// this function cn be called with zero, the use of
...
// the function will return the number of trailing zeroes in the type.
// if the given number is zero, this function will return the number if bits
template<Unsigned IntType>
inline constexpr int count_trailing_zeroes_safe(IntType value) {
  if (value == 0)
    return 8 * sizeof(IntType);
...
// in the IntType.
template<Unsigned IntType>
inline constexpr count_leading_zeroes_safe(IntType values) {
  if (value == 0)
    return 8 * sizeof(IntType);
