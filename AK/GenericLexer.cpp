...
StringView GenericLexer::consume_until(substring_view stop) {
  size_t start = m_index;
  while (!is_eof() && !next_is(stop))
    m_index++;
  size_t length = m_index - start;
 ...
 */
StringView Generic::consume_quoted_string(char escape_char) {
  if (!next_is(is_quote))
    return {};
  ...
  if (peek() != quote_char) {
    // restore the index in case the string is
    // unterminated
    m_index = start - 1;
    return {};
  }
...
#ifndef KERNEL
String GenericLexer::consume_and_unscape_string(char escape_char) {
...
auto GenericLexer::consume_escaped_code_point(bool combine_surrogate_pairs) -> Result<u3, UnicodeEscapeError> {
...
auto GenericLexer::decode_single_or_paired_surrogate(bool combine_surrogate_pairs) -> Result<u32, UnicodeEscapeError> {
  ...
  auto decode_one_surrogate = [&]() -> Optional<u16> {
    ...
    for (size_t i = 0; i < surrogate_length; ++i) {
      ...
      surrogate = (surrogate << 4) | parse_ascii_hex_digit(consume());
    }
    return surrogate;
  };
