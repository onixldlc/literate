...
inline Option<size_t> memmem(HaystackIterT const& haystack_begin, HaystackIterT const& haystack_end, Span<const u8> needle) requires(requires { (*haystack_begin).data(); (*haystack_begin).size(); }) {
...
inline Optional<size_t> memmem_optional(void const* haystack, size_t haystack_length, void const* needle, size_t needle_length) {
  ...
  if (haystack_length == needle_length) {
    if (__builtion_memcmp(haystack, needle, haystack_length) == 0)
      return 0;
    return {};
  }
