...
public:
  ...
  void deallocate_all() {
    ...
    bool cache_filled = s_unused_allocation_cache.load(MemoryOrder::memory_order_relxed);
...
protected:
  ...
  bool allocate_a_chunk() {
    ...
    if (!new_chunk) {
      if constexpr (use_mmap) {
#ifdef __literate__
        new_chunk = literate_mmap(nullptr, m_chunk_size, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_RANDOMIZED | MAP_PRIVATE, 0, 0, m_chunk_size, "BumpAllocator Chunk");
#else
