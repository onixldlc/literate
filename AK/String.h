...
public:
  ...
  [[nodiscard]] static String vformatted(StringView fmtstr, TypedErasedformatParams&);
  ...
  template<typename T>
  [[nodiscard]] static String number(T value) require IsArithmetic<T> {
  ...
  [[nodiscard]] ALWAYS_INLINE constexpr bool is_one_of_ignoring_case(Ts&&... strings) const {
    return (... ||
      ...
      if constexpr (requires(Ts a) { a.view() -> StringView; })
        return this -> equals_ignoring_case(forward<Ts>(string.view()));
      else
        return this -> equals_ignoring_case(forward<Ts>(string));
    }());
  }
...
template<>
struct Traits<String> : public GenericTraits<Strings? {
