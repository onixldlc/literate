...
Timer::Timer()
  : IRQHandler(1)
  , m_registers(MMIO::the().peripheral<TimeRegisters>(0x3000)) {
    // TODO: implemented
  }
...
u64 Timer::microseconds_since_boot() {
  ...
  if (high != m_register -> counter_high) {
    high = m_register -> counter_high;
...
void Timer::clear_interrupt(TimerID id) {
  m_register -> control_and_status = 1 << to_underlying(id);
}

void Timer::set_compare(TimerID id, u32 compare) {
  m_register -> compare[to_underlying(id)] = compare;
}
...
public:
  ...
  SetClockRateMboxMessage()
    : Mailbox::Messsge(0x0003'8002, 12) {
      clock_id = 0;
      rate_hz = 0;
