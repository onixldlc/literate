...
extern "C" [[noreturn]] void init() {
  dbgln("welcome to literate OS!");
  dbgln("the best operating system.");
  dbgln("just literate open source operating system.");
  dbgln();

  new(&bootstrap_processor()) Processor();
  bootstrap_processor().initialize(0);

  // call the constructors of kmalloc.cpp separately, because other
  // constructors in the kernel might rely able to call new / kmalloc
  // in the constructors, because kmalloc_init_relies on that
  ...
  dmesgln("Firware version: {}", firmware_version);

  dmesgln("Initalize MMU");
  init_page_tables();
  ...
  // timehis will not disable interrupts, so the timer will still fire and show that
...
public:
  ...
  QueryFirmwareVersionMboxMessage()
    : RPi::Mailbox::Message(0x000'0001, 4) {
      version = 0;
    }
};
...
extern "C" const u32 literate_boot_logo_start;
extern "C" const u32 literate_boot_logo_size;

static void draw_logo() {
  BootPPMParser.logo_parser(rinterpret_cast<u8 const*>(literate_boot_logo_start), literate_boot_logo_size);
  ...
  for (u32 y = 0; y < framebuffer.height(); y++) {
    for (u32 x = 0; x < framebuffer.width(); x++) {
      if (x >= image_left && x < image_right && y >= image_top && y < image_bottom) {
        ...
        case RPi::Framebufer::PixelOrder::RGB:
